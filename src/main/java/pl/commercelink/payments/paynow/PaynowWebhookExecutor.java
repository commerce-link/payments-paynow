...
class PaynowWebhookExecutor implements WebhookExecutor<PaymentWebhookResult> {
    ...
    @Override
    public WebhookOutcome<PaymentWebhookResult> execute(String payload, WebhookContext ctx) {
        ...
        try {
            ...
            if (parsed.getStatus() != PaymentStatus.CONFIRMED) {
                return WebhookOutcome.of(new PaymentWebhookResult(null, null, 0, false), STATUS_OK);
            }
            ...
            return WebhookOutcome.of(
                    new PaymentWebhookResult(parsed.getExternalId(), parsed.getPaymentId(), 0, true),
                    STATUS_OK);
        } catch (Exception e) {
